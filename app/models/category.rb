class Category < ActiveRecord::Base
  ...
  def party_positions
    ...
    ind_category_yes = Position.joins(:legislator, vote: {bill: {subjects: :category}}).where(categories: {name: self.name}, legislators: {party: "I"}, positions: {vote_position: "Yes"})
    ind_category_no = Position.joins(:legislator, vote: {bill: {subjects: :category}}).where(categories: {name: self.name}, legislators: {party: "I"}, positions: {vote_position: "No"})
    ind_category_total = Position.joins(:legislator, vote: {bill: {subjects: :category}}).where(categories: {name: self.name}, legislators: {party: "I"})
